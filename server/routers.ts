    ...
    login: publicProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (!user || !user.password) {
          // Use constant-time comparison placeholder to avoid timing attacks
        ...
        const storedHash = user.password.startsWith("sha256:") ? user.password.slice(7) : null;

        // Support both hashed passwords (new) and plain text (migration path)
